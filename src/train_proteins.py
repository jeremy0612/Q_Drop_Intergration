"""PROTEINS experiment wrapper over the shared Torch graph training core."""
...
def build_parser():
    ...
    return build_train_parser(
        description="Train QGCN on PROTEINS",
...
def plot_results(fold_results, output_path: Path):
    ...
    axes[0].bar(range(1, len(fold_accuracies) + 1), fold_accuracies, color="#55A868", edgecolor="black")
    ...
    plt.suptitle(f"QGCN on PROTEINS — {mean_accuracy:.3f}±{std_accuracy:.3f} accuracy", fontweight="bold")
    plt.tight_layout()
...
def main():
    ...
    print("Loading PROTEINS from HuggingFace (cached after first download)...")

    result_payload = train_dataset("proteins", config, device=device)
    legacy_metrics = build_legacy_metrics(result_payload, config)

    metrics_path = output_dir / "proteins_metrics.json"
    with open(metrics_path, "w", encoding="utf-8") as metrics_file:
        json.dump(legacy_metrics, metrics_file, indent=2)
    print(f"[+] Metrics saved to {metrics_path}")

    plot_results(result_payload["folds"], output_dir / "proteins_results.png")
